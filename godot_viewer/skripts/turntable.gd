...
# -------------------------------------------------------------------------
# Adjust Model on Startup
# -------------------------------------------------------------------------

#Berechnet den gewichteten Schwerpunkt des Modells, wenn gewollt
...
#Teil der weighted Berechnung, rekursiv
...
#Skaliert modell auf standardisierte Größe
...
func start_explosion(selected_part: MeshInstance3D):
	...
	for mesh in parent_branch.keys():
		if mesh != selected_part:
			...
			mesh_original_positions[mesh] = mesh.global_position
			mesh_explosion_targets[mesh] = target_pos
			exploded_meshes[mesh] = true
			#print("Mesh in explosiontargets: ", mesh_explosion_targets)
	#Erster Ansatz um den Parent irgendwie seperat ausgewählt, über model_hierarchy, hat allerdings lange nicht funktionier. Vielleicht einfach den parent von dem man kommt in seperater Variable zwischenspeichern und dann seperat wie die meshes verschieben.
	#var direct_parent = find_direct_parent_in_current_level(main.current_level, selected_part)
	#print(direct_parent)
	push_state()
...
func start_implosion():
	pop_state()
	for mesh in mesh_explosion_targets.keys():
		mesh_explosion_targets[mesh] = mesh.global_transform.origin
	exploded_meshes.clear() #wird benötigt um zu tracken welche Teilmodelle gerade wegexplodiert sind, für UI Funktionen, gerade noch nicht mit hierarchie kompatibel
	if is_animation_active:
		return
	is_animation_active = true
	explosion_elapsed = 0.0
...
#Setzt zu allerletzt visuell die neue Position für die Zentrierung des neufokussierten Modellteils
...
#Findet die jeweiligen anderen kompletten branches basierend auf der momentanen ebene und wählt diese für die wegexplosion aus.
...
#Behebt Kollisionen zwischen Mesh-Zielpositionen indem die betroffenen Modelle verschoben werden
